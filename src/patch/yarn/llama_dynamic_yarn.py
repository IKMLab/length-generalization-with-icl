...
class LlamaDynamicYaRNScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def forward(self, x, position_ids):
        ...
        if seq_len > self.scaling_factor * self.max_position_embeddings:
            ...
            scaling_factor = seq_len / (self.scaling_factor * self.max_position_embeddings)

            pos_freqs = self.base**(torch.arange(0, self.dim, 2).float().to(x.device) / self.dim)
            ...
            inv_freq_mask = (1 - _yarn_linear_ramp_mask(low, high, self.dim // 2).float()).to(
                x.device) * self.extrapolation_factor
