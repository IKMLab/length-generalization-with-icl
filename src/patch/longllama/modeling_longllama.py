...
def rotate_one(x, cos, sin, position_ids):
    if len(position_ids.shape) != 2 or x.shape[0] != position_ids.shape[
            0] or x.shape[-2] != position_ids.shape[1]:
        raise ValueError(
            f"Position ids shoud have shape [bsz, seq_len] got {position_ids.shape}"
        )
...
class LongLlamaAttention(nn.Module):
    ...
    def __init__(self,
                 ...
                 mem_config: Optional[LongLlamaMemConfig] = None):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        if position_ids.shape != (key_states.shape[0], 1, key_states.shape[-2],
                                  1):
            raise ValueError(
                "position_ids should match batch and seq_len of the input")

        mem_no_local_cache = self.mem_config is not None and past_key_value is None and (
            not use_cache)
        ...
        if mem_no_local_cache:
            ...
            if use_positionals:
                ...
                rfst_key_states = rotate_as_if_first(key_states,
                                                     self.rotary_emb)
            else:
        ...
        if past_key_value is not None:
            ...
            if attention_mask.shape[-1] != key_states.shape[
                    -2] and attention_mask.shape[-2] != query_states.shape[-2]:
                raise ValueError(
                    "attention_mask should be provided for all key_states in local context"
                )
            ...
            if key_states.shape[-2] > self.max_cache:
                ...
                if mem_and_local_cache:
                    ...
                    if use_positionals:
                        rfst_drop_keys = rotate_as_if_first(
                            drop_keys, self.rotary_emb)
                    else:
                    ...
                    if mem_cache is None:
                        ...
                        mem_cache = mem_apply_update(
                            prev_mem_cache=mem_cache,
                            new_mem_content=mem_update,
        ...
        past_key_value = (key_states, value_states,
                          position_ids) if use_cache else None

        kv_seq_len = key_states.shape[-2]

        if use_positionals:
            cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
            rel_pos_ids = position_ids - torch.min(
                position_ids, dim=-2, keepdim=True)[0]
            rel_pos_ids = rel_pos_ids.squeeze(3).squeeze(1)

            query_states = rotate_one(query_states, cos, sin,
                                      rel_pos_ids[:, -query_states.shape[-2]:])
                ...
                if self.config.torch_attention:
                    if mem_cache is not None:
                        attn_keys = torch.concat([
                            key_states[:, beg_h:end_h],
                            mem_cache.keys[:, beg_h:end_h].to(key_states.dtype)
                        ],
                                                 dim=-2)
                        attn_values = torch.concat(
                            [
                                value_states[:, beg_h:end_h],
                                mem_cache.values[:, beg_h:end_h].to(
                                    value_states.dtype)
                            ],
                            dim=-2,
                        )
                        ...
                        mem_mask = torch.broadcast_to(
                            mem_mask, (mem_mask.shape[0], mem_mask.shape[1],
                                       end_q - beg_q, mem_mask.shape[3]))
                        attn_mask = torch.concat(
                            [attention_mask[:, :, beg_q:end_q], mem_mask],
                            dim=-1)
                    ...
                    attn_weights = torch.matmul(
                        ...
                        key_states[:, beg_h:end_h].transpose(
                            2, 3)) / math.sqrt(self.head_dim)

                    if attn_weights.size() != (bsz, end_h - beg_h,
                                               end_q - beg_q, kv_seq_len):
                        raise ValueError(
                    ...
                    min_value = (torch.finfo(attn_weights.dtype).min
                                 if -1000000.0 < torch.finfo(
                                     attn_weights.dtype).min else -1000000.0)
                    ...
                    if mem_cache is not None:
                        ...
                        mem_attn_weights = torch.matmul(
                            query_states[:, beg_h:end_h, beg_q:end_q],
                            mem_cache.keys[:, beg_h:end_h].transpose(2, 3).to(
                                key_states.dtype),
                        ) / math.sqrt(self.head_dim)

                        assert mem_mask.shape[2] == 1
                        mem_attn_weights = mem_attn_weights + mem_mask
                        min_value = (torch.finfo(mem_attn_weights.dtype).min
                                     if -1000000.0 < torch.finfo(
                                         mem_attn_weights.dtype).min else
                                     -1000000.0)
                        mem_attn_weights = torch.max(
                            mem_attn_weights,
                            torch.tensor(min_value,
                                         device=mem_attn_weights.device,
                                         dtype=mem_attn_weights.dtype),
                        )

                        attn_weights = torch.concat(
                            [attn_weights, mem_attn_weights], dim=-1)
        ...
        if mem_no_local_cache:
            if mem_cache is not None:
                mem_cache = mem_apply_update(prev_mem_cache=mem_cache,
                                             new_mem_content=mem_update,
                                             mem_config=self.mem_config)
            else:
                mem_cache = mem_update
...
class LongLlamaDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LongLlamaAttention(config=config,
        ...
        self.input_layernorm = LongLlamaRMSNorm(config.hidden_size,
                                                eps=config.rms_norm_eps)
        self.post_attention_layernorm = LongLlamaRMSNorm(
...
def _prepare_pos_ids(past_key_values, batch_size, input_length, device):
    if past_key_values is not None:
        ...
        next_pos = torch.max(past_key_values[0][2].view(batch_size, -1),
                             dim=-1)[0] + 1
    ...
    position_ids = torch.arange(0,
                                input_length,
                                dtype=torch.long,
                                device=device).view(1, input_length)
...
class LongLlamaModel(LongLlamaPreTrainedModel):
    ...
    def __init__(self, config: LongLlamaConfig):
        ...
        for layer_id in range(config.num_hidden_layers):
            if layer_id in self.mem_layers:
                layer = LongLlamaDecoderLayer(config,
                                              mem_config=self.mem_config)
            else:
        ...
        self.norm = LongLlamaRMSNorm(config.hidden_size,
                                     eps=config.rms_norm_eps)
    ...
    @add_start_docstrings_to_model_forward(LONGLLAMA_COMMON_INPUTS_DOCSTRING,
                                           LONGLLAMA_MODEL_INPUTS_DOCSTRING)
    def forward(
        self,
        ...
        if position_ids is None:
            ...
            position_ids = _prepare_pos_ids(past_key_values, batch_size,
                                            seq_length, device)
        else:
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if mem_cache is not None and idx not in self.mem_layers:
                raise ValueError(
                    "Memory cache provided for a non-memory leayer")
        ...
        for layer in past_key_values:
            if len(layer) != 6:
                ...
                if memk.shape[-2] != 0:
                    ...
                    mem_caches += (LongLlamaMemCache(keys=memk,
                                                     values=memv,
                                                     masks=memm), )
                else:
                    mem_caches += (None, )
...
def _handle_long_input(
    ...
    if past_key_values is not None and use_cache is False:
        raise ValueError(
            "past_key_values it not None should imply use_cache == True")
    ...
    if position_ids.shape != (batch_size, input_length):
        raise ValueError(
            f"Shape of position_ids [{position_ids}] should match [{batch_size, input_length}]"
        )

    if attention_mask is not None:
        attention_mask = attention_mask[..., -(initial_past_key_values_length +
                                               input_length):]
        if attention_mask is not None and (attention_mask.shape != (
                batch_size, initial_past_key_values_length + input_length)):
            raise ValueError(
    ...
    if mem_input_length > 0:
        for i in range(0, mem_input_length, context_window_length):
            ...
            outputs = model(
                input_ids=input_ids[..., beg:end]
                if input_ids is not None else None,
                attention_mask=attention_mask[..., attn_beg:attn_end]
                if attention_mask is not None else None,
                position_ids=position_ids[..., beg:end],
                past_key_values=past_key_values,
                inputs_embeds=inputs_embeds[..., beg:end, :]
    ...
    attention_mask = attention_mask[
        ..., -attn_length:] if attention_mask is not None else None
    ...
    outputs = model(
        ...
        past_key_values=past_key_values,
        inputs_embeds=inputs_embeds[..., beg:, :]
        if inputs_embeds is not None else None,
        use_cache=use_cache,
        output_attentions=output_attentions,
        output_hidden_states=output_hidden_states,
    ...
    outputs = BaseModelOutputWithPast(
        last_hidden_state=torch.concat(
            [x.last_hidden_state for x in outputs_list], dim=-2),
        past_key_values=past_key_values,
        hidden_states=hidden_states,
...
class LongLlamaForCausalLM(LongLlamaPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(LONGLLAMA_COMMON_INPUTS_DOCSTRING,
                                           LONGLLAMA_ADD_INPUTS_DOCSTRING)
    ...
    def forward(
        ...
        last_context_length = (last_context_length
                               if last_context_length is not None else
                               self.config.last_context_length)
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (output_hidden_states
                                if output_hidden_states is not None else
                                self.config.output_hidden_states)
