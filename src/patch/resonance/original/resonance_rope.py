...
from .utils import find_correction_range, get_mscale, linear_ramp_mask
    ...
    def compute_freqs(self, seq_len, device):
        ...
        for i in range(self.dim // 2):
            if seq_len >= self.r_wavelengths[i].item():
                ...
                current_freq = repeat(t_i * self.r_inv_freq[i],
                                      'l -> (repeat l)',
...
class ResonanceYaRNScaledRotaryEmbedding(ResonanceRotaryEmbedding):
    ...
    def _register_buffers(self, device):
        ...
        inv_freq_mask = (1 - linear_ramp_mask(low, high, self.dim // 2).float().to(device)
        ...
        self.mscale = float(get_mscale(self.scale) *
                            self.attn_factor)  # Get n-d magnitude scaling corrected for interpolation
